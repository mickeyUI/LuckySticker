...
import OrderStatisticsCharts from "@/components/OrderStatisticsCharts";
...
type OrderItemFullProp = OrderItemProp & {
  category: CategoryNameProp;
};
type PosterNameProp = {
  name: string;
  category: CategoryNameProp;
};
type CategoryNameProp = {
  name: string;
};
...
const statusInducators = [
  ...
  { stat: "ordered", bgColor: "bg-blue-400/50" },
];
...
type PostProps = {
  ...
  setCategories: React.Dispatch<React.SetStateAction<CategoryProp[]>>;
  preview: string;
...
type OrdersProps = {
  orders: OrderProp[];
  ordersStat?: OrderStatsProp;
  setSelectedOrder: React.Dispatch<React.SetStateAction<OrderProp | undefined>>;
...
function Orders({
  ...
  return (
    <section className="rounded-xl border border-zinc-800 bg-zinc-900 p-6 space-y-8 w-full">
      <div className="grid grid-cols-2 md:grid-cols-4  justify-around gap-5">
        ...
        <button
          // onClick={}
          className="flex flex-col gap-4 stat-box w-full p-5 text-blue-500"
        >
            ...
            {orders.map((order: OrderProp) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-4">{order.delivery_location}</td>
...
export default function AdminPage() {
  ...
  const [selectedOrder, setSelectedOrder] = useState<OrderProp | undefined>(
    undefined,
  );
  const [openOrderDetails, setOpenOrderDetails] = useState(false);
  const [ordersStat, setOrdersStat] = useState<OrderStatsProp | undefined>(
    undefined,
  );
  const [selectedMenu, setSelectedMenu] = useState<string>("Post");
  const [orderItems, setOrderItems] = useState<OrderItemFullProp[]>([]);
  ...
  const getOrders = async () => {
    ...
    const rows = (data ?? []) as OrderProp[];
    setOrders(rows);
    const pending = rows.filter((order) => order.status === "pending").length;
    const ordered = rows.filter((order) => order.status === "ordered").length;
    const delivered = rows.filter(
      (order) => order.status === "delivered",
    ).length;
    const canceled = rows.filter((order) => order.status === "canceled").length;
    setOrdersStat({
      pending: pending,
      ordered: ordered,
      delivered: delivered,
  ...
  const loadOrderItems = async () => {
    ...
    *,
    ...
    setOrderItems((data ?? []) as OrderItemFullProp[]);
  };
  const xs = async () => {
  ...
  const processedOrderItems = orderItems.map((item) => ({
    ...item,
    poster_name: item.poster_name?.name || "Unknown",
    category_name: item.poster_name?.category?.name || "Uncategorized",
  }));
  ...
  const MenuCards = [
    ...
    },
    {
      name: "Statistics",
      Component: (
        <OrderStatisticsCharts
          orders={orders || []}
          orderItems={processedOrderItems}
          title="Poster Order Analytics"
        />
      ),
    },
  ];

  const DiplayCard = MenuCards.find((card) => card.name == selectedMenu);
  return (
